    ...
    <div>
      ...
      <div *ngIf="!isLoading" class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <div class="flex gap-4">
          <a routerLink="/admin/orders" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm font-medium transition-colors">Vérifier les paiements</a>
          <a routerLink="/admin/products" class="inline-flex items-center px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 text-sm font-medium transition-colors">Ajouter un abonnement</a>
        </div>
      </div>
    </div>
  `
...
export class AdminDashboardComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    try {
      // Le compilateur sait maintenant qu'il s'agit d'un tableau de type 'any'
      const orders: any[] = await this.orderService.getAllOrders() || [];
